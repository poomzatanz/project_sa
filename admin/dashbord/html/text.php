...
<div id="wrapper">
    ...
    <?php
	 $conn = new mysqli("db4free.net","poomzaadekd2023","poomzaadekd2023","dekdanalatic2023");
      ...
      $totol3 = array();
      $totols = array();
	$sql = "SELECT count(*) as num ,SUBSTRING(time, 4, 2) as month FROM pre_web WHERE text_acc = 'positive' GROUP BY SUBSTRING(time, 4, 2) ORDER by SUBSTRING(time, 4, 2);";
    ...
    $sql = "SELECT count(*) as num ,SUBSTRING(time, 4, 2) as month FROM pre_web WHERE text_acc = 'negative' GROUP BY SUBSTRING(time, 4, 2) ORDER by SUBSTRING(time, 4, 2);";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
    ...
    $sql = "SELECT count(*) as num ,SUBSTRING(time, 4, 2) as month FROM pre_web WHERE text_acc = 'normal' GROUP BY SUBSTRING(time, 4, 2) ORDER by SUBSTRING(time, 4, 2);";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
    ...
    $sql = "SELECT count(*) as num ,text_acc as month FROM pre_web GROUP BY text_acc;";
    $result = $conn->query($sql);
    /* If there are results from database push to result array */
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex no-block">
                            <div>
                                <h2></h2>
                                <h1 data-shadow='dang!'>วิเคราะห์เนื้อหากระทู้</h1>
                            </div>
                            <div class="ml-auto">
