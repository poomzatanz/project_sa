		...
		<table id="customers" >
			...
			<tbody>
				...
				if($value!=""){
					...
					$sql_forms = $db->query("SELECT pk_web FROM web  where text_acc != 'negative' and topic_acc != 'negative' ");
					$pages->items_total = $sql_forms->num_rows;
					$pages->mid_range = 10;
					$pages->paginate();	
					$q = "SELECT * FROM web  where text_acc != 'negative' and topic_acc != 'negative' ORDER BY CONCAT(SUBSTRING(time, 7, 2),SUBSTRING(time, 4, 2),SUBSTRING(time, 1, 2),SUBSTRING(time, 10, 5)) DESC  ".$pages->limit." ";
					//echo $q;
					$result	=	$db->query($q);
